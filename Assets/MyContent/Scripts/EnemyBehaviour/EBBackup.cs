...
public class EBBackup : MonoBehaviour
{
                         ...
                         // TODO: Give enemies sight so player can be detected within cone of vision whilst stealthed (then begin chase)
    {
        ...
        if (player.GetComponent<PMBackup>().crouched == false) // Follow player (player is detected)
        {
            running = true;
            transform.LookAt(target); // Rotate enemies to face player character
            transform.Translate(Vector3.forward * 3 * Time.deltaTime);
        }
        else if (player.GetComponent<PMBackup>().crouched == true) // Remain idle (player is undetected)
        {
            running = false;
        }
